# TODO: Translation updated at 2023-06-11 22:49

translate czech strings:

    # game/screens.rpy:248
    old "Back"
    new "Zpět"

    # game/screens.rpy:249
    old "History"
    new "Historie"

    # game/screens.rpy:250
    old "Skip"
    new "Přeskakování"

    # game/screens.rpy:251
    old "Auto"
    new "Auto"

    # game/screens.rpy:252
    old "Save"
    new "Uložit"

    # game/screens.rpy:253
    old "Q.Save"
    new "Rychlé Uložení"

    # game/screens.rpy:254
    old "Q.Load"
    new "Rychlé Načtení"

    # game/screens.rpy:255
    old "Prefs"
    new "Možnosti"

    # game/screens.rpy:296
    old "Start"
    new "Začít"

    # game/screens.rpy:304
    old "Load"
    new "Načíst"

    # game/screens.rpy:306
    old "Preferences"
    new "Možnosti"

    # game/screens.rpy:310
    old "End Replay"
    new "Ukončit Replay"

    # game/screens.rpy:314
    old "Main Menu"
    new "Hlavní Menu"

    # game/screens.rpy:316
    old "About"
    new "O Hře"

    # game/screens.rpy:321
    old "Help"
    new "Nápověda"

    # game/screens.rpy:326
    old "Quit"
    new "Odejít"

    # game/screens.rpy:369
    old "Ren'Py 7+ Edition"
    new "Ren'Py 7+ Edice"

    # game/screens.rpy:466
    old "Return"
    new "Návrat"

    # game/screens.rpy:550
    old "[config.version!t]\n"
    new "[config.version!t]\n"

    # game/screens.rpy:554
    old "Updated Character Art"
    new "Nové Obrázky Postav"

    # game/screens.rpy:555
    old "Deji"
    new "Deji"

    # game/screens.rpy:559
    old "Original Character Art"
    new "Původní Obrázky Postav"

    # game/screens.rpy:560
    old "Derik"
    new "Derik"

    # game/screens.rpy:566
    old "Updated Background Art"
    new "Nové Obrázky Pozadí"

    # game/screens.rpy:567
    old "Mugenjohncel"
    new "Mugenjohncel"

    # game/screens.rpy:571
    old "Original Background Art"
    new "Původní Obrázky Pozadí"

    # game/screens.rpy:572
    old "DaFool"
    new "DaFool"

    # game/screens.rpy:578
    old "Music By"
    new "Hudba"

    # game/screens.rpy:579
    old "Alessio"
    new "Alessio"

    # game/screens.rpy:585
    old "Update Written By"
    new "Nový Příběh"

    # game/screens.rpy:586
    old "Lore"
    new "Lore"

    # game/screens.rpy:590
    old "Originally Written By "
    new "Původní Příběh"

    # game/screens.rpy:591
    old "mikey (ATP Projects)"
    new "mikey (ATP Projects)"

    # game/screens.rpy:594
    old "\nMade with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]"
    new "\nVytvořeno s {a=https://www.renpy.org/}Ren'Py{/a} verze [renpy.version_only]"

    # game/screens.rpy:596
    old "[renpy.license!t]"
    new "[renpy.license!t]"

    # game/screens.rpy:638
    old "Page {}"
    new "Strana {}"

    # game/screens.rpy:638
    old "Automatic saves"
    new "Automatické ukládání"

    # game/screens.rpy:638
    old "Quick saves"
    new "Rychlé ukládání/načítání"

    # game/screens.rpy:680
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%A, %B %d %Y, %H:%M"

    # game/screens.rpy:680
    old "empty slot"
    new "prázdná pozice"

    # game/screens.rpy:697
    old "<"
    new "<"

    # game/screens.rpy:700
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:703
    old "{#quick_page}Q"
    new "{#quick_page}R"

    # game/screens.rpy:709
    old ">"
    new ">"

    # game/screens.rpy:771
    old "Display"
    new "Zobrazení"

    # game/screens.rpy:772
    old "Window"
    new "V Okně"

    # game/screens.rpy:773
    old "Fullscreen"
    new "Celá Obrazovka"

    # game/screens.rpy:778
    old "Unseen Text"
    new "Neviděný Text"

    # game/screens.rpy:779
    old "After Choices"
    new "Po Volbách"

    # game/screens.rpy:780
    old "Transitions"
    new "Přechody"

    # game/screens.rpy:789
    old "Language"
    new "Jazyk"

    # game/screens.rpy:799
    old " "
    new " "

    # game/screens.rpy:817
    old "Text Speed"
    new "Rychlost Textu"

    # game/screens.rpy:821
    old "Auto-Forward Time"
    new "Čas Auto-Přeskakování Dopředu"

    # game/screens.rpy:828
    old "Music Volume"
    new "Hlasitost Hudby"

    # game/screens.rpy:835
    old "Sound Volume"
    new "Hlasitost Zvuků"

    # game/screens.rpy:841
    old "Test"
    new "Test"

    # game/screens.rpy:845
    old "Voice Volume"
    new "Hlasitost Hlasů"

    # game/screens.rpy:856
    old "Mute All"
    new "Ztlumit Vše"

    # game/screens.rpy:975
    old "The dialogue history is empty."
    new "Historie dialogů je prázdná."

    # game/screens.rpy:1041
    old "Keyboard"
    new "Klávesnice"

    # game/screens.rpy:1042
    old "Mouse"
    new "Myš"

    # game/screens.rpy:1045
    old "Gamepad"
    new "Ovladač"

    # game/screens.rpy:1058
    old "Enter"
    new "Enter"

    # game/screens.rpy:1059
    old "Advances dialogue and activates the interface."
    new "Posune dialog a aktivuje rozhraní."

    # game/screens.rpy:1062
    old "Space"
    new "Mezerník"

    # game/screens.rpy:1063
    old "Advances dialogue without selecting choices."
    new "Posune dialog bez přeskočení voleb."

    # game/screens.rpy:1066
    old "Arrow Keys"
    new "Šipky"

    # game/screens.rpy:1067
    old "Navigate the interface."
    new "Navigace v rozhraní."

    # game/screens.rpy:1070
    old "Escape"
    new "Escape"

    # game/screens.rpy:1071
    old "Accesses the game menu."
    new "Vstoupí do herního menu."

    # game/screens.rpy:1074
    old "Ctrl"
    new "Ctrl"

    # game/screens.rpy:1075
    old "Skips dialogue while held down."
    new "Přeskočí dialog když podržen."

    # game/screens.rpy:1078
    old "Tab"
    new "Tab"

    # game/screens.rpy:1079
    old "Toggles dialogue skipping."
    new "Vypíná/Zapíná přeskakování dialogu."

    # game/screens.rpy:1082
    old "Page Up"
    new "Page Up"

    # game/screens.rpy:1083
    old "Rolls back to earlier dialogue."
    new "Přetočí na dřívější dialog."

    # game/screens.rpy:1086
    old "Page Down"
    new "Page Down"

    # game/screens.rpy:1087
    old "Rolls forward to later dialogue."
    new "Přetočí na pozdější dialog."

    # game/screens.rpy:1091
    old "Hides the user interface."
    new "Skryje uživatelské rozhraní."

    # game/screens.rpy:1095
    old "Takes a screenshot."
    new "Udělá screenshot (fotku obrazovky)."

    # game/screens.rpy:1099
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "Zapíná/Vypíná asistenci {a=https://www.renpy.org/l/voicing}samo-hlasu{/a}."

    # game/screens.rpy:1103
    old "Opens the accessibility menu."
    new "Otevře menu usnadnění přístupu."

    # game/screens.rpy:1109
    old "Left Click"
    new "Levé Tlačítko Myši"

    # game/screens.rpy:1113
    old "Middle Click"
    new "Prostření Tlačítko Myši"

    # game/screens.rpy:1117
    old "Right Click"
    new "Pravé Tlačítko Myši"

    # game/screens.rpy:1121
    old "Mouse Wheel Up\nClick Rollback Side"
    new "Kolečko Myši Nahoru\nKliknutí Přetočení Strana"

    # game/screens.rpy:1125
    old "Mouse Wheel Down"
    new "Kolečko Myši Dolů"

    # game/screens.rpy:1132
    old "Right Trigger\nA/Bottom Button"
    new "Pravý Trigger\nDolní A Tlačítko"

    # game/screens.rpy:1136
    old "Left Trigger\nLeft Shoulder"
    new "Levý Trigger\nLevý Shoulder"

    # game/screens.rpy:1140
    old "Right Shoulder"
    new "Pravý Shoulder"

    # game/screens.rpy:1144
    old "D-Pad, Sticks"
    new "D-Pad, Páčky"

    # game/screens.rpy:1148
    old "Start, Guide"
    new "Start, Guide"

    # game/screens.rpy:1152
    old "Y/Top Button"
    new "Tlačítko Y/Horní Tlačítko"

    # game/screens.rpy:1155
    old "Calibrate"
    new "Kalibrovat"

    # game/screens.rpy:1220
    old "Yes"
    new "Ano"

    # game/screens.rpy:1221
    old "No"
    new "Ne"

    # game/screens.rpy:1267
    old "Skipping"
    new "Přeskakování"

    # game/screens.rpy:1488
    old "Menu"
    new "Menu"

